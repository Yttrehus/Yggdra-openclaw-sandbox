...
**B: TransportIntra Arkiv** — Komplet projektmappe. Kildeindex: /root/Yggdra/REF.transportintra/TI_KOMPLET_KILDEINDEX.md (519 linjer, dækker ALLE kilder). Alt TI-output i `REF.transportintra/`. Rør IKKE `REF.transportintra/`.
...
**Metode:** 2 subagents:
- Sub A → `DLR.research-architecture/_audit_index.md`: Læs RESEARCH_INDEX.md (allerede kategoriseret), brug som baseline
- Sub B → `DLR.research-architecture/_audit_sizes.md`: `find /root/Yggdra/research/ -name '*.md' -exec wc -l {} + | sort -rn | head -40` + `head -3` af de 20 største
- Orchestrator merger til `DLR.research-architecture/_audit.md`
**Output:** `DLR.research-architecture/_audit.md` — tabel [fil, kategori, one-line, linjer]
...
**Input:** `DLR.research-architecture/_audit.md`
**Output:**
- `DLR.research-architecture/INDEX.md` — crown jewels, kategorier, "hvornår du bruger den"
- `DLR.research-architecture/CONTEXT.md` — med kill condition ("hvis INDEX.md ikke bruges i 20 sessioner, slet projektet")
...
- Sub A → `REF.transportintra/_scan/sessions_chatlogs.md`: De 5 chatlogs med flest hits (2026-02-08: 168, 2026-02-15: 55, 2026-02-19: 24, 2026-02-18: 21, 2026-02-21: 17). Brug `grep -n 'transportintra\|TI\|rute 256\|sortering' /root/Yggdra/data/chatlogs/YYYY-MM-DD.md | head -30` per fil, derefter `sed -n 'X,+30p'` for kontekst. Udtyæk: dato, hvad der blev gjort, beslutninger
- Sub B → `REF.transportintra/_scan/sessions_exports.md`: ChatGPT, Claude App, Grok samtalerne fra kildeindexet sektion 2-4. Læs de 3 vigtigste (Copy and paste info, Gjentatte problemer, Google sheets workflow) med `head -200` + `grep`
- Orchestrator merger kronologisk til `REF.transportintra/_scan/sessions.md`
**Output:** `REF.transportintra/_scan/sessions.md` — kronologisk, med datoer, beslutninger
...
**Metode:** 2 subagents:
- Sub A → `REF.transportintra/_scan/technical_app.md`: `find /root/Yggdra/app/ -name '*.js' -exec wc -l {} + | sort -rn | head -30` (KUN top 30, IKKE alle). Sub-apps: `ls -la app/classic/ app/v2/ app/redesign/ app/command-center/ app/mindmap/`. Features fra featurePanel.js (head -50)
- Sub B → `REF.transportintra/_scan/technical_infra.md`: Scripts (7 TI-scripts fra kildeindex), n8n workflows (sektion 12), data/ (routes stats: `ls data/routes/ | wc -l`, `ls data/routes/ | head -5 && ls data/routes/ | tail -5`), API reference (head -30), Docker (fra kildeindexet sektion "Docker/Infrastructure")
- Orchestrator merger til `REF.transportintra/_scan/technical.md`
**Output:** `REF.transportintra/_scan/technical.md`
...
**Output:** `REF.transportintra/PROGRESS.md`
...
**Format — som LIB.ydrasil/INDEX.md:**
...
**Output:** `REF.transportintra/INDEX.md`
...
**Input:** PROGRESS.md + INDEX.md + TI_KOMPLET_KILDEINDEX.md + /root/Yggdra/REF.transportintra/CONTEXT.md
...
**Overordnet:** `REF.transportintra/CONTEXT.md` — samlet state, hvad er TI, hvad er done, hvad mangler, link til subprojects
...
**Done:** `find REF.transportintra/ -type f | wc -l` > 20
...
ssh root@72.62.61.51 "cat /root/Yggdra/yggdra-pc/REF.transportintra/INDEX.md"

# Filtal
ssh root@72.62.61.51 "find /root/Yggdra/yggdra-pc/REF.transportintra/ -type f | wc -l"
ssh root@72.62.61.51 "find /root/Yggdra/yggdra-pc/DLR.research-architecture/ -type f | wc -l"
